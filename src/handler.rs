...
pub trait NormalHandler<D1, D2, E>: Send + Sync{
	fn handle<'a, 'b, 'c>(&self, req: &mut Request<'a, 'b, 'c>, data: D1) -> HandlerResult<D2, E>;
}
...
pub trait ParamHandler<D1, D2, E>: Send + Sync{
	fn handle_param<'a, 'b, 'c>(&self, req: &mut Request<'a, 'b, 'c>, data: D1, param: Option<&str>) -> HandlerResult<D2, E>;
}

impl<D1, E> NormalHandler<D1, D1, E> for (){
	fn handle<'a, 'b, 'c>(&self, req: &mut Request<'a, 'b, 'c>, data: D1) -> HandlerResult<D1, E>{
		req.next(data)
	}	
} 

impl<T, D1, D2, E> NormalHandler<D1, D2, E> for T
...
where T: Send + Sync + for<'a, 'b, 'c> Fn(&mut Request<'a, 'b, 'c>, D1, Option<&str>) -> HandlerResult<D2, E> {
	fn handle_param<'a, 'b, 'c>(&self, req: &mut Request<'a, 'b, 'c>, data: D1, param: Option<&str>) -> HandlerResult<D2, E>{
		(*self)(req, data, param)
	}
}
